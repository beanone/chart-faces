...
public class RightAxisTool extends CartesianAxisTool {
	...
	RightAxisTool(final IChartToolFactory factory, final IChartPart part,
	...
	protected void addLabels(final IDataElement element) {
		...
		final double x = getAxisPart().getTickHeight()
				+ getChart().getPadding()
		...
		if (labelsIterable != null) {
			label.addProperty(SVGConstants.SVG_X, String.valueOf(x));
	...
	protected IStringBuilder createAxisLinePath() {
		...
		final IStringBuilder sb = new NumberRoundingStringBuilder()
				.append('M').append("0 0v").append(viewBox.getHeight());
		return sb;
	}
	...
	protected IElement createCaptionTextElement(final IElement captionElement) {
		final IPartSpec specs = getPartSpecs();
		final Box viewBox = specs.getViewBox();
		final int x = viewBox.getWidth() - getChart().getPadding()
		...
		captionElement.addProperty(SVGConstants.SVG_TRANSFORM,
				SVGConstants.ROTATE_PLUS_90 + x + ' ' + y + ')');
		return element;
	}
